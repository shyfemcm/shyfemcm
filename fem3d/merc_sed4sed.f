...
!       assigne old value to water (sw,POMw) and sediment (silt, POM) variables
...
!___________ Compute weigthed particle density [g cm-3], porosity [-], Bulk density [g(s+w) cm-3]    

       Pdens = ((1.25*p_POM)+(2.65*(100.0 - p_POM)))/100.0  ![g(s)/cm3(s)]   
       por = 1.0 - (DryD/Pdens)        ! [L(w)/L(s+w)]
       BulkD = por + DryD              ! [g/cm3] 
!
!____________ Input critical shear for erosion tCE ____________
!
!       tCE  = 0.65                     ![Pa]

!        write(*,*) tCE, 'tCE read from mercury.f'
        

!___________ Compute resuspension multiplier(dme/dt) from BulkD _________  
!___________ from Hwang and Metha (1989) in Tetra Tech (2002) _________         
!
...
!
!       write(665,*) k,es,logdme,BulkD,OC_mg_g
!       write(666,*) k,taub,dme2, por 
!       write(667,*) k,dme2, OC_mg_g 

!___________ Resuspension Occurrence _________________________________ 
...
!___________ Compute Resuspension Fluxes and rates _________________________ 
...
!       if (ipext(k) .EQ. 2284) then 
!       write(888,*) Vr, Er, Sres, Pres, taub
!       end if 

!       if (taub .GT. 0.4) then
!       write (779,*) ipext(k),Vr, Er, Sres, Pres, taub
!       end if
!___________ Compute Burial Velocities and Fluxes ____________________________
       
       Bvels = (ds_gm2s-rs_gm2s)/silt    ![m s-1]
       Bvelp = (dp_gm2s-rp_gm2s)/POM 
!        
!       write(*,*) 'dep', ds_gm2s,'res', rs_gm2s, k       
...
!       write(486,*) Dssink_sum, Dpsink_sum, 'sed4Merc_Sed'
       end if 
...
!        Bsflux=0.0
!        Bpflux=0.0
!___________ Compute sediment thickness variations ____________________________   
...
!       
!____________Positive burial push sediment below the active layer__________________
!____________Negative burial is equal to net resuspension thus not included________
...
!        write (988,*) (CS(m), m=1,nstate),'k',k,'SEdSolid var beforer'
!        write (987,*) (C(m), m=1,nstate),'k',k, 'SEdwater var beforer' 
...
!        call merc_euler (2,dt,sed_vol_old,cs,cold,cds)
...
!       write(443,*) 'Siltw<0',c(1), 's4m_sedbef kext=',ipext(k)
       else if (c(2) .LE. 0.0) then
!       write(447,*) 'POMw<0',c(2), 's4m_sedbef kext=',ipext(k) 
       write(*,*) 'POMw<0',c(2),wdep, 's4m_sedaft kext=',ipext(k)
	stop 'error stop... conz<0 (ggu)'
       end if

!      call merc_euler (2,dt,
!     +                        wat_vol, c,cold,cd)
...
!       write(443,*) 'Siltw<0',c(1), 's4m_sedaft kext=',ipext(k)
       else if (c(2) .LE. 0.0) then
!       write(447,*) 'POMw<0',c(2), 's4m_sedaft kext=',ipext(k)       
       write(*,*) 'POMw<0',c(2),wdep,'s4m_sedaft kext=',ipext(k) 
      end if
...
!        write (988,*) (CS(m), m=1,nstate),'k',k,'SEdSolid var after' !' Transformations'
!        write (987,*) (C(m), m=1,nstate),'k',k,'SEdwater var after' 
     
!        write (777,*) wat_vol,cd,k, 'wat_vol and water derivative cd'
!        write (888,*) wat_vol,cds,k, 'wat_vol and sed derivative cds'
